...
setwd("~/Git/SethupathyLab/FLC/Sample Log/20210428_Mitochondrial_Dan")
```
...
sample_log <- read_csv('~/Git/SethupathyLab/FLC/Sample Log/FLC Primary Sample Log.csv') %>%
  separate(Sample, c("individual", "string"), sep = "_", remove = F)

### This is the ChROseq table from Tim's Cell Reports paper
chro_samples <- read_csv("~/Git/SethupathyLab/FLC/Sample Log/ChROseq_samples.csv")
chro_samples <- substr(names(chro_samples)[-1], 5, 14)

### These are the RNAseq samples
rna <- read_tsv("~/Git/SethupathyLab/FLC/Sample Log/pFLC01-34_CountTable_DESeq2Normalized_noTCGA_20200202.txt")
...
sample_matched.data <- sample_log %>% 
  select("Sample", "ChRO-seq", "ChROseq included in Cell Reports", "FLCs sequenced with (mRNA)", "mRNA included in Cell Reports") %>% 
